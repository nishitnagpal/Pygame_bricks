...
function draw(player, elapsedTime, ball, bricks, score, showStartMessage, paused, endMessage, finalTime, winMessage) {
    ...
    if (showStartMessage && !endMessage && !winMessage) {
        ...
        drawMultilineText(ctx, startText, "white", WIDTH / 2, HEIGHT / 2);
    }

    if (paused && !showStartMessage) {
        const pauseText = `Game Paused\n\n Press Space to Resume`;
        drawMultilineText(ctx, pauseText, "purple", WIDTH / 2, HEIGHT / 2);
    }

    if (endMessage) {
        const lostText = `You Lost!\nTime Taken: ${Math.round(finalTime)}s\nFinal Score: ${score}\n\nBetter Luck Next Time! \nPress Enter to Play Again`;
        drawMultilineText(ctx, lostText, "red", WIDTH / 2, HEIGHT / 2);
    }

    if (winMessage) {
        const winText = `You Won!\nTime Taken: ${Math.round(finalTime)}s\nFinal Score: ${score}\n\nPress Enter to Play Again`;
        drawMultilineText(ctx, winText, "green", WIDTH / 2, HEIGHT / 2);
    }
}
...
function main() {
    ...
    document.addEventListener('keydown', (event) => {
      if (gameStarted && !paused && ballMoving){
